...
genotypez = {'c9orf'}%{'APOE' 'c9orf' 'HLA' 'GBA'};
...
%         %% List sample sizes
        ...
        subplot(2,3,4)
        ylabel(['N animals = ' num2str(length(unique(Data_breeders.AnimalID)))])
        xlabel(['N weights = ' num2str(Rows(Data_breeders))])
        set(gca,'XTick',[],'YTick',[])%current axis or x and y
        set(gca,'Color',[.75 .75 .75])
% %         pubify_figure_axis(20,20);
%         D_B= Data_breeders;
%         y= D_B(:, (["Genotype";"Sex";"AnimalID"]));
%         ysorted= sortrows(y);
% 
%          NB_Female_c9_NT= ysorted(1:869,:);
%          NB_Male_c9_NT= ysorted(870:1951,:);
%        NB_Female_c9_TG= ysorted(1952:2825, :);
%         NB_Male_c9_TG= ysorted(2826:4229, :);
%         
%        
%         
%         D_NB= Data_nonbreeders;
%         z= D_NB(:, (["Genotype";"Sex";"AnimalID"]));
%       zsorted= sortrows(z);
%       Female_c9_NT= zsorted(1:39, :);
%         Male_c9_NT= zsorted(40:190,:);
%        Female_c9_TG= zsorted(191:323, :);
%         Male_c9_TG= zsorted(324: 404, :);
